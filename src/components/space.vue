<template>
	<div class="tables-view" :class="{
		...
		<div class="grid-controls">
			...
			<input id="grid-width" type="range" v-model="gridWidth" min="10" max="40">
			<label for="grid-height">Grid height: {{gridHeight}}</label>
			<input id="grid-height" type="range" v-model="gridHeight" min="10" max="40">
		</div>
		<div class="tables-block">
			<div class="tables-grid" ref="grid" :style="{
				width: getGridWidth,
				height: getGridHeight,
				backgroundSize: ''.concat(gridSize, 'px'),
				backgroundPosition: ''.concat((gridSize / 2) * -1, 'px ', (gridSize / 2) * -1, 'px')
			}">
				<div
					class="table-wrapper"
					:style="{
			...
			<div class="tables-control">
				<div class="tables-control__new-tables">
					...
					<div class="table circle table--placeholder" @click="onCreateNewTable(true)"></div>
				</div>
				<div class="tables-control__actions">
					<button @click="onReset" :disabled="isBtnsDisabled">
						Отмена
					</button>
					<button @click="onSave" :disabled="isBtnsDisabled">
						Сохранить
					</button>
				</div>
			</div>
...
const getGridWidth = computed(() => {
	return ''.concat((gridSize.value * gridWidth.value) + (gridSize.value / BG_GRID_SIZE), 'px')
})

const getGridHeight = computed(() => {
	return ''.concat((gridSize.value * gridHeight.value) + (gridSize.value / BG_GRID_SIZE), 'px')
})
...
function onTableResize(e, table) {
	const {clientX, clientY, updateOnResize, updateOnResizeY} = initializeData(e, table) 
	if (resizeYItemIndex.value !== -1) {
		if (clientY === 0) {
			return;
		}
...
const initializeData = (e, table) => {
	...
	const updateOnResizeX = () => {
		const leftPosition = gridBoundingPosition.value.left + getTableNode().offsetLeft
	...
	const updateOnResizeY = () => {
		const topPosition = gridBoundingPosition.value.top + getTableNode().offsetTop
	...
	return { updateOnDrag, updateOnResize, updateOnResizeY, clientX, clientY, getTableNode };
}
const changeTableType = (table) => {
	table.isRounded = !table.isRounded
}
const onCreateNewTable = (rounded = false) => {
	_tables.value.push(transformTableData({
		width: 3,
		height: 3,
...
const getAsidePxUnits = (v) =>
	''.concat((Math.round((gridSize.value * v) / gridSize.value) * gridSize.value) + gridSize.value / BG_GRID_SIZE, 'px');
</script>
...
.table-resize-bottom {
	...
	height: 2px;
	width: 100%;
	background-color: rgba(rgb(80, 80, 80), .6);
	cursor: s-resize;
}
.delete {
	position: absolute;
